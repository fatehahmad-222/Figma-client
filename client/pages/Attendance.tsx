...
const baseEmployees: Employee[] = [
  ...
  { id: 3, name: "Priya Sharma", role: "HR Coordinator", department: "Human Resources", store: "City Store", avatar: "PS", avatarClass: "bg-green-100 text-green-600", worked: "22/23" },
...
function StatusIcon({ status }: { status: AttendanceStatus }) {
  ...
  if (status === "present") return <CheckCircle2 className={`${common} text-green-500`} />;
...
function SelectField({ label, value, onChange, children }: { label: string; value: string; onChange: (value: string) => void; children: React.ReactNode }) {
  return (
    <label className="relative block text-xs font-medium text-slate-500">
      ...
      <select value={value} onChange={(event) => onChange(event.target.value)} className="w-full appearance-none rounded-lg border border-slate-200 bg-white px-3 py-2.5 pr-8 text-sm font-normal text-slate-700 outline-none focus:border-green-500 focus:ring-2 focus:ring-green-100">
        {children}
      </select>
...
export default function Attendance() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800">
      ...
      <div className={isModalOpen ? "blur-sm" : ""}>
        <main className="mx-auto max-w-[1600px] px-5 py-5 sm:px-8">
          <div className="mb-5 flex flex-wrap items-center justify-between gap-4">
            ...
            <button onClick={openModal} type="button" className="flex items-center gap-2 rounded-lg bg-green-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-green-700"><Plus className="h-4 w-4" /> Add Record</button>
          </div>

          <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
            <div className="flex flex-col gap-3 lg:flex-row">
              <div className="flex min-w-0 flex-1 items-center gap-2 rounded-lg border border-slate-200 px-3 py-2.5 focus-within:border-green-500 focus-within:ring-2 focus-within:ring-green-100">
            ...
            {filtersVisible && <div className="mt-5 border-t border-slate-100 pt-4">
              ...
              <div className="mt-4 flex items-center justify-end gap-4"><button onClick={applyFilters} type="button" className="rounded-lg bg-green-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-green-700">Apply Filters</button><button onClick={() => { setFilters({ employee: "All Employees", department: "All Departments", month: "July", year: "2026", store: "All Stores" }); setAppliedFilters({ employee: "All Employees", department: "All Departments", month: "July", year: "2026", store: "All Stores" }); setCurrentPage(1); }} type="button" className="text-sm text-slate-500 hover:text-slate-800 hover:underline">Reset Filters</button></div>
            </div>}
          </section>
          ...
          <section className="rounded-xl border border-slate-200 bg-white shadow-sm">
            <div className="hidden overflow-hidden lg:block">
              <table className="w-full table-fixed border-collapse text-xs">
                <thead><tr className="border-b border-slate-200 bg-slate-50 text-center text-[10px] font-bold text-slate-500"><th className="w-[230px] border-r border-slate-200 bg-slate-50 px-4 py-3 text-left">EMPLOYEES</th>{Array.from({ length: daysInMonth }, (_, index) => { const day = index + 1; const weekday = new Date(Number(appliedFilters.year), months.indexOf(appliedFilters.month), day).toLocaleDateString("en-US", { weekday: "short" }).toUpperCase(); return <th key={day} className="px-0.5 py-2"><div>{weekday}</div><div className="mt-1 text-slate-700">{day}</div></th>; })}</tr></thead>
                <tbody>{pageEmployees.length === 0 ? <tr><td colSpan={daysInMonth + 1} className="px-4 py-12 text-center text-sm text-slate-500">No employees found.</td></tr> : pageEmployees.map((employee) => <tr key={employee.id} className="group border-b border-slate-100 last:border-0 hover:bg-slate-50"><td className="border-r border-slate-100 bg-white px-3 py-2 group-hover:bg-slate-50"><div className="flex items-center gap-2.5"><div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-[10px] font-bold ${employee.avatarClass}`}>{employee.avatar}</div><div className="min-w-0"><div className="truncate font-semibold text-slate-700">{employee.name}</div><div className="truncate text-[10px] text-slate-400">{employee.role} <span className="font-semibold text-green-600">{employee.worked}</span></div></div></div></td>{Array.from({ length: daysInMonth }, (_, index) => <td key={index} className="px-0.5 py-3 text-center"><div className="flex justify-center"><StatusIcon status={getStatus(employee.id, index + 1, overrides)} /></div></td>)}</tr>)}</tbody>
              </table>
            </div>
            <div className="divide-y divide-slate-100 lg:hidden">{pageEmployees.length === 0 ? <p className="px-4 py-12 text-center text-sm text-slate-500">No employees found.</p> : pageEmployees.map((employee) => <article key={employee.id} className="p-4"><div className="flex items-center gap-3"><div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-[10px] font-bold ${employee.avatarClass}`}>{employee.avatar}</div><div className="min-w-0"><p className="truncate text-sm font-semibold text-slate-700">{employee.name}</p><p className="truncate text-xs text-slate-400">{employee.role} · <span className="font-semibold text-green-600">{employee.worked}</span></p></div></div><div className="mt-3 grid grid-cols-7 gap-1 text-center">{Array.from({ length: daysInMonth }, (_, index) => <div key={index} className="hidden sm:block"><span className="text-[9px] text-slate-400">{index + 1}</span><div className="mt-1 flex justify-center"><StatusIcon status={getStatus(employee.id, index + 1, overrides)} /></div></div>)}<div className="col-span-7 flex items-center justify-between text-xs text-slate-500 sm:hidden"><span>Attendance summary</span><span className="font-medium text-slate-700">{employee.worked} days worked</span></div></div></article>)}</div>
          </section>
      ...
      {isModalOpen && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-3 backdrop-blur-sm sm:p-4"><div role="dialog" aria-modal="true" className="w-full max-w-[500px] rounded-2xl bg-white p-4 shadow-2xl sm:p-6"><div className="mb-5 flex items-start justify-between"><div><h3 className="text-lg font-bold text-slate-900">Add Attendance Record</h3><p className="mt-1 text-sm text-slate-500">Log attendance for an employee</p></div><button onClick={() => setIsModalOpen(false)} type="button" aria-label="Close modal" className="rounded-full p-1 text-slate-400 hover:bg-slate-100 hover:text-slate-700"><X className="h-5 w-5" /></button></div><div className="space-y-4"><label className="relative block text-xs font-semibold text-slate-700">Employee<select value={newRecord.employeeId} onChange={(event) => setNewRecord({ ...newRecord, employeeId: event.target.value })} className="mt-1.5 w-full appearance-none rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm font-normal text-slate-500 outline-none focus:border-green-500"><option value="">Select Employee</option>{baseEmployees.map((employee) => <option key={employee.id} value={employee.id}>{employee.name}</option>)}</select><ChevronDown className="pointer-events-none absolute bottom-2.5 right-3 h-4 w-4 text-slate-400" /></label><label className="block text-xs font-semibold text-slate-700">Date<input type="date" value={newRecord.date} onChange={(event) => setNewRecord({ ...newRecord, date: event.target.value })} className="mt-1.5 w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm font-normal text-slate-600 outline-none focus:border-green-500" /></label><div><span className="text-xs font-semibold text-slate-700">Attendance Status</span><div className="mt-2 flex flex-wrap gap-2">{(["present", "absent", "half-day", "late", "on-leave"] as AttendanceStatus[]).map((status) => <button key={status} type="button" onClick={() => setNewRecord({ ...newRecord, status })} className={`rounded-full border px-3 py-1.5 text-xs font-medium capitalize transition ${newRecord.status === status ? "border-green-500 bg-green-50 text-green-700" : "border-slate-200 text-slate-500 hover:border-slate-300"}`}>{status.replace("-", " ")}</button>)}</div></div><div className="grid grid-cols-2 gap-3"><label className="block text-xs font-semibold text-slate-700">Clock In<input type="time" value={newRecord.clockIn} onChange={(event) => setNewRecord({ ...newRecord, clockIn: event.target.value })} className="mt-1.5 w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm font-normal outline-none focus:border-green-500" /></label><label className="block text-xs font-semibold text-slate-700">Clock Out<input type="time" value={newRecord.clockOut} onChange={(event) => setNewRecord({ ...newRecord, clockOut: event.target.value })} className="mt-1.5 w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm font-normal outline-none focus:border-green-500" /></label></div><label className="block text-xs font-semibold text-slate-700">Notes (Optional)<textarea value={newRecord.notes} onChange={(event) => setNewRecord({ ...newRecord, notes: event.target.value })} placeholder="Add any notes here..." rows={3} className="mt-1.5 w-full resize-none rounded-lg border border-slate-200 px-3 py-2.5 text-sm font-normal outline-none focus:border-green-500" /></label></div><div className="mt-6 flex justify-end gap-3 border-t border-slate-100 pt-5"><button onClick={() => setIsModalOpen(false)} type="button" className="rounded-lg border border-slate-200 px-5 py-2.5 text-sm font-medium text-slate-600 hover:bg-slate-50">Cancel</button><button onClick={submitRecord} type="button" className="rounded-lg bg-green-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-green-700">Add Record</button></div></div></div>}
    </div>
  );
}
