    ...
    <div className="min-h-screen bg-slate-50 text-slate-800">
      ...
      <main className="mx-auto max-w-[1600px] px-5 py-5 sm:px-8">
        <div className="mb-5 flex items-end justify-between gap-4">
          ...
          <button type="button" onClick={() => setIsModalOpen(true)} className="flex items-center gap-2 rounded-lg bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-emerald-700"><Plus className="h-4 w-4" /> Add Leave Application</button>
        </div>
        ...
        <section className="mb-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
          <div className="flex flex-col gap-3 lg:flex-row">
            ...
            <button type="button" onClick={() => setFiltersVisible((visible) => !visible)} className="flex items-center justify-center gap-2 rounded-lg bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-emerald-700"><Filter className="h-4 w-4" />{filtersVisible ? "Hide Filters" : "Show Filters"}</button>
          </div>
          {filtersVisible && <div className="mt-5 border-t border-slate-100 pt-4"><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">{(["employee", "department", "month", "year", "store"] as const).map((key) => <label key={key} className="relative block text-xs font-medium text-slate-500">{key[0].toUpperCase() + key.slice(1)}<select value={filters[key]} onChange={(event) => setFilters({ ...filters, [key]: event.target.value })} className="mt-1.5 w-full appearance-none rounded-lg border border-slate-200 bg-white px-3 py-2.5 pr-8 text-sm font-normal text-slate-700 outline-none focus:border-green-500"><option>{key === "employee" ? "All Employees" : key === "department" ? "All Departments" : key === "month" ? "July" : key === "year" ? "2026" : "All Stores"}</option>{key === "employee" ? employees.map((employee) => <option key={employee.id}>{employee.name}</option>) : key === "department" ? [...new Set(employees.map((employee) => employee.department))].map((department) => <option key={department}>{department}</option>) : key === "month" ? ["June", "July", "August"].map((month) => <option key={month}>{month}</option>) : key === "year" ? ["2025", "2026", "2027"].map((year) => <option key={year}>{year}</option>) : <><option>Main Store</option><option>City Store</option><option>Airport Store</option></>} </select><ChevronDown className="pointer-events-none absolute bottom-2.5 right-2.5 h-4 w-4 text-slate-400" /></label>)}</div><div className="mt-4 flex items-center justify-end gap-4"><button type="button" onClick={() => { setAppliedFilters(filters); setFiltersVisible(false); }} className="rounded-lg bg-emerald-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-emerald-700">Apply Filters</button><button type="button" onClick={resetFilters} className="text-sm text-slate-500 hover:underline">Reset Filters</button></div></div>}
        </section>
