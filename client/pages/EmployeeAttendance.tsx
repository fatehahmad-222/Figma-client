...
const EmployeeAttendance: React.FC = () => {
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-800"><TopBar /><main className="mx-auto max-w-[1600px] px-5 py-5 sm:px-8 sm:py-6">
    ...
    <section className="flex flex-col gap-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm sm:flex-row sm:items-center"><div className="flex h-16 w-16 items-center justify-center rounded-full bg-green-50 text-lg font-bold text-green-700">{employee.initials}</div><div className="flex-1"><div className="flex flex-wrap items-center gap-2"><h1 className="text-2xl font-bold text-slate-900">{employee.name}</h1><span className="rounded-full bg-green-100 px-2.5 py-0.5 text-xs font-semibold text-green-700">{employee.status}</span></div><p className="mt-1 text-sm text-slate-500">{employee.role} · {employee.department} · {employee.shift}</p><p className="mt-1 text-xs text-slate-400">{employee.id}</p></div><button className="self-start rounded-lg border border-slate-300 px-4 py-2 text-sm font-medium hover:bg-slate-50 sm:self-center">Edit Profile</button></section>
