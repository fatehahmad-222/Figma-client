...
const attendanceData = [
  ...
  { date: "05-07-2026", status: "Leave", clockIn: "On Leave", clockOut: "On Leave", clocked: "-", clockedPct: 0, overtime: "-" },
...
function StatusPill({ status }: { status: string }) {
  if (status === "Present") return <span className="font-semibold text-green-600">Present</span>;
  if (status === "Leave") return <span className="rounded bg-rose-50 px-2 py-0.5 font-semibold text-rose-600">Leave</span>;
  if (status === "Holiday") return <span className="rounded bg-blue-50 px-2 py-0.5 font-semibold text-blue-600">Holiday</span>;
  if (status === "Late") return <span className="rounded bg-rose-50 px-2 py-0.5 font-semibold text-rose-600">Late</span>;
  return <span className="font-medium text-slate-400">Not Marked</span>;
}
...
const EmployeeAttendance: React.FC = () => {
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-800"><TopBar /><main className="mx-auto max-w-[1600px] px-5 py-5 sm:px-8 sm:py-6">
    ...
    <nav className="mt-3 flex gap-7 border-b border-slate-200 text-sm">{tabs.map((tab) => <NavLink key={tab} end={tab === "General"} to={tabPath(tab)} className={({ isActive }) => `border-b-2 px-0 py-3 ${isActive ? "border-green-600 font-semibold text-green-600" : "border-transparent text-slate-500 hover:text-slate-700"}`}>{tab}</NavLink>)}</nav>
    <div className="mt-4">
      <div className="mb-4 flex items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-2"><label className="relative"><select value={selectedYear} onChange={(e) => setSelectedYear(e.target.value)} className="appearance-none rounded-lg border border-slate-200 bg-white py-2 pl-3 pr-8 text-sm outline-none"><option>2025</option><option>2026</option></select><ChevronDown className="pointer-events-none absolute right-2 top-2.5 h-4 w-4 text-slate-400" /></label><label className="relative"><select value={selectedMonth} onChange={(e) => setSelectedMonth(e.target.value)} className="appearance-none rounded-lg border border-slate-200 bg-white py-2 pl-3 pr-8 text-sm outline-none"><option>January</option><option>February</option><option>March</option><option>April</option><option>May</option><option>June</option><option>July</option><option>August</option><option>September</option><option>October</option><option>November</option><option>December</option></select><ChevronDown className="pointer-events-none absolute right-2 top-2.5 h-4 w-4 text-slate-400" /></label></div>
      </div>
      <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        {[["PRESENT / WORKING DAYS", "2/6 Days", "text-slate-900"], ["TOTAL OFFICE TIME", "16 hrs", "text-green-600"], ["TOTAL WORKED TIME", "2 hrs 14 mins", "text-slate-900"], ["LATE", "1 Days (16.67%)", "text-rose-600"], ["HALF DAYS", "0", "text-rose-600"], ["LEAVES IN MONTH", "0", "text-slate-900"]].map(([label, value, color], i) => <div key={i} className="rounded-lg border border-slate-100 bg-white p-3 shadow-sm"><p className="text-[10px] font-semibold uppercase text-slate-400">{label}</p><p className={`mt-1 text-lg font-bold ${color}`}>{value}{label === "TOTAL WORKED TIME" && <span className="ml-1 text-xs font-normal text-slate-400">(13.96%)</span>}</p></div>)}
      </div>
